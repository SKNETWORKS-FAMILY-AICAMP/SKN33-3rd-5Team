...
조건 추출 단독 응답 시간·처리량·GPU peak 메모리는 raw 로그(`artifacts/evaluation/`)에
기록만 되어 있고 아직 정식 지표로 코드에 반영하지 않았습니다. 측정된 실행 시간은
모델 로딩(약 4~5초)이 포함된 값이라 그대로 처리량으로 쓰지 않습니다. 자세한 내용은
[A40 최종 검증 기록](docs/validation/2026-08-31-final-a40-defca71.md)을 참고하세요.
